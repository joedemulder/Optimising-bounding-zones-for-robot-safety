...
arm_speed = 0.4 # Speed for arm movement
...
joint_trajectories = calculate_human_trajectory(target_arm_angles_basic, arm_speed) # CHANGE CHOSEN ARM MOVEMENT HERE
...
collision_indices = [num_joints - 3]  # Links chosen to cover the end-effector (wrist)
...
capsules_inner = []
collision_dimensions_small = [(0.1, 0.1)]
for index, (radius, height) in zip(collision_indices, collision_dimensions_small):
    ...
    capsules_inner.append((capsule_id, index))

capsules_middle = []
collision_dimensions_medium = [(0.175, 0.175)]
...
capsules_outer = []
collision_dimensions_large = [(0.25, 0.25)]
for index, (radius, height) in zip(collision_indices, collision_dimensions_large):
    collision_shape_id = p.createCollisionShape(shapeType=p.GEOM_CAPSULE, radius=radius, height=height)
    visual_shape_id = p.createVisualShape(shapeType=p.GEOM_CAPSULE, radius=radius, length=height, rgbaColor=[1, 0, 0, 0.11])
    ...
    capsules_outer.append((capsule_id, index))
...
try:
    ...
    while 1:
        ...
        for joint_configs in joint_trajectory:
            ...
            for capsule_id, link_index in capsules_outer:
            ...
            for capsule_id, link_index in capsules_inner:
                link_state = p.getLinkState(robotID, link_index)
                p.resetBasePositionAndOrientation(capsule_id, link_state[4], link_state[5])
                if p.getContactPoints(capsule_id, humanID):
                    max_speed = 0
    ...
    # Save data to CSV
    with open('speed_data_script2.csv', 'w', newline='') as csvfile:
